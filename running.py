...
first_chain = basic_metropolis_hastings(-5, mcmc_iterations )

simply_plot_the_chain(first_chain, [100, 200, 300] )

first_poisson_chain = poisson_3_MCMC(0, mcmc_iterations)
simply_plot_the_chain(first_poisson_chain, fmt_plt="kD")
plt.bar(*np.unique(first_poisson_chain, return_counts=True))
plt.show()

second_poisson_chain = poisson_3_MCMC(50, mcmc_iterations)
